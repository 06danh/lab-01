...
```{r Ex_f}
visitors %>% 
  filter(100000 > n_2022, n_2022 > 50000) %>%
  count()
```
...
# No amount due to error in dataset, untidy data. NAs in data cannot be used to calculate mean or median as not dbl data type.
...
**Write your answer here**
Data set is weighted so that the highest values are significantly further from median then the low values, which raises the mean but does not affect the median compared to an even distribution
...
visitors_with_nations %>%
  count(nation)
```
IQR  most reliable for England because it has highest number of attractions (240). Fairly reliable for Scotland as number of attractions is 97, unreliable for Northern Ireland or Wales because number of attractions is too low.
